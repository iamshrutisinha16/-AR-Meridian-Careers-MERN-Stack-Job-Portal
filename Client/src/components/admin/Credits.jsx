...
import { 
    Bell, Check, ChevronDown, ChevronUp, Sparkles, ArrowLeft 
} from 'lucide-react'
...
const CreditsPricing = () => {
    ...
    // Real user credits se state initialize karein (Fallback 0 rakha hai taaki fake 200 na aaye)
    const [userCredits, setUserCredits] = useState(0)
    const [openFaq, setOpenFaq] = useState(0) // First FAQ open by default

    // Jab Redux state ya user object load ho, toh real credits set karein
    useEffect(() => {
    ...
    const handleBuyPlan = async (planName, creditAmount, price) => {
        try {
            // Yahan aap apni backend API call integrate kar sakte hain (jaise Razorpay / Stripe / Payment Gateway)
            // Example: const response = await axios.post('/api/payment/buy-credits', { planName, creditAmount, price });
            
            // Success hone par local state ya redux update karein
    ...
    const faqData = [
        ...
        },
        {
            ...
            a: "For bulk purchases exceeding 50,000 credits, you can click on 'Contact Sales' below to get a customized enterprise quotation tailored to your hiring volume."
        },
        {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans flex flex-col">
            ...
            {/* Top Navigation Bar */}
            <header className="bg-white border-b border-gray-200 px-4 md:px-8 py-3.5 flex justify-between items-center sticky top-0 z-20 shadow-xs">
                <div className="flex items-center gap-3">
                    <button 
                        onClick={() => navigate(-1)} 
                        className="p-2 rounded-lg hover:bg-gray-100 text-gray-700 transition cursor-pointer flex items-center gap-1.5 text-sm font-semibold"
                    >
                        <ArrowLeft className="w-4 h-4" /> Back
                    </button>
                    <h2 className="text-base font-bold text-gray-900 border-l border-gray-200 pl-3">Pricing & Plans</h2>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2 shadow-2xs">
                        <span className="text-amber-500">🪙</span> {userCredits} Credits Available
                    </div>
                    <button className="text-gray-500 hover:text-gray-700 p-2 rounded-full hover:bg-gray-100 cursor-pointer transition relative">
            ...
            {/* Main Content Area */}
            <main className="flex-1 w-full max-w-7xl mx-auto px-4 md:px-8 py-10 space-y-16">
                ...
                <div className="text-center space-y-3 max-w-2xl mx-auto">
                    ...
                    <p className="text-sm md:text-base text-gray-500">
                        Select a plan that fits your hiring requirements and speed up recruitment with powerful candidate boosts.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
                    ...
                    {/* Card 1 */}
                    <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-200 shadow-xs flex flex-col justify-between hover:border-blue-400 transition">
                    ...
                    {/* Card 3 (Recommended) */}
                    <div className="bg-white rounded-2xl p-6 md:p-8 border-2 border-blue-600 shadow-md flex flex-col justify-between relative">
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xs">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="font-bold text-gray-900 text-base md:text-lg">Looking for something else?</h3>
                            <p className="text-xs md:text-sm text-gray-500">Get a custom quote based on your exact hiring volume and enterprise needs.</p>
                        </div>
                    </div>
                    <button 
                        onClick={() => alert("Our sales representative will reach out to you shortly!")}
                        className="px-6 py-3 border border-gray-300 hover:border-gray-400 font-bold text-gray-700 rounded-xl text-xs md:text-sm transition cursor-pointer whitespace-nowrap bg-gray-50 hover:bg-gray-100"
                    >
                        Contact sales
                    </button>
                </div>
                        ...
                        {faqData.map((item, index) => {
                            ...
                            return (
                                <div 
                                    ...
                                    className="bg-white border border-gray-200 rounded-xl p-4 md:p-5 cursor-pointer shadow-2xs transition hover:border-gray-300"
                                >
